...
| CodeGen-350M-mono | 350M | 10.2% | — | No | 577B (mixed) | Same param count, no distillation (HF model card) |
...
distillation, no FIM, 10.2% HumanEval pass@1 — corrected from earlier 12.8%
estimate per HF model card). The realistic target for distillation is **+2-5
points** over the base model per recent KD literature (arXiv:2409.12586).

**Key insight from phi-1 (arXiv:2306.11644)**: phi-1-small (350M) achieved
45% HumanEval using only 7B tokens of **synthetic textbook-quality data**
generated by GPT-3.5. Albor trains on codeparrot-clean (raw GitHub Python) —
far lower data quality. This is the single largest factor limiting our
HumanEval ceiling. The distillation stage from Qwen3-Coder-30B partially
compensates by generating high-quality synthetic completions.

**Scaling context**: Chinchilla-optimal for 350M is ~7B tokens (20:1 ratio).
Albor's 5.08B tokens (14.5:1) is slightly below optimal. However, modern
practice "overtrains" small models well beyond Chinchilla — Llama 3 uses
1875:1. More tokens on better data would help, but data quality dominates
quantity at this scale (phi-1 lesson).

**Perplexity → HumanEval mapping** (estimated from literature):

| val_ppl range | Expected HumanEval | Model capability |
|---------------|-------------------|------------------|
| > 200 | 0% | Unstructured predictions |
| 100-200 | 0-3% | Basic structure, mostly invalid |
| 50-100 | 3-8% | Some valid functions |
...
| CodeGen-350M-mono | 350M | 10.2% | No (never submitted) |
